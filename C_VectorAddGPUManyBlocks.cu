...
__global__ void addVectorsGPU(float*, float*, float*, int); //need to add pointers for the functions to match because that is what we are dealing with
bool check(float*, int, float); //add float to match function later
...
void setUpDevices()
{
	BlockSize.x = 256; //our new block size is 256
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = ((N - 1 ) / BlockSize.x) + 1; //change grid size to equation we found last class
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	int id = blockIdx.x * blockDim.x + threadIdx.x; //calculate to find placement of thread, also found last class
	
	if(id < n) //if loop prevents the threads from accessing area outside of it's allocated array
	{
		c[id] = a[id] + b[id];
	}
}
...
bool check(float *c, int n, float tolerance)
{
	...
	if(percentError < tolerance) //lowercase because we passed tolerance into the function so we need to actually use the parameter 
	{
		return(true);
	}
	else 
...
void cleanUp() //lowercase to match prototype
{
...
int main()
{
	...
	cleanUp(); //lowercase to match prototype
