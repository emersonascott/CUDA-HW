...
/*
 ...
 1. Add pointers to the "__global__ void addVectorsGPU" prototype to match the function we call later. This allows it to point to many numbers in our GPU vectors instead of a singular number.
 2. Change all of the "cleanUp" functions to lowercase so they match
 3. Add a "float" argument to the "bool check" prototype to match the later function that is called and has 3 arguments
 4. Change from "abs" to "fabs" because we are working with floating-point numbers. Needs to include <math.h> for this as well
 5. Change "GridSize.x" to 64 instead of using out equation
 6. Add "int stride" so that each thread can potentially do multiple elements instead of just one. It figures out that each stride is 16384 by multiplying 256 x 64
 7. Add a "for" loop to allow the program to handle vectors much larger than the number of threads
...
#include <stdio.h>
#include <math.h>
#include <stdlib.h> //because we are using malloc, free, and exit
...
__global__ void addVectorsGPU(float*, float*, float*, int); //change the prototype to be pointers to match the later function
bool check(float*, int, float); //add float because we call "float tolerance" in our main function later
...
void setUpDevices()
{
	...
	GridSize.x = 64; //This is the assignment change
	GridSize.y = 1;
	GridSize.z = 1;
			 //Now we have 256 threads per block, 64 total blocks, so in total we have 16384 threads
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	int stride = blockDim.x * gridDim.x; //This does 256 x 64 so stride = 16384. Using this function allows each thread to potentially do multiple elements instead of just one
	
	#pragma unroll //experimentation with loop unrolling!
	for(int i = id; i < n; i += stride) //This allows the program to handle vectors much larger than the number of threads
	{
		if(i < n) // Making sure we are not working on memory we do not own. //Change to lowercase n because we do not want to use the global variable
		{
			float aValue = sqrtf(cosf(a[i]) * cosf(a[i])
                 	         + a[i] * a[i]
                  		 + sinf(a[i]) * sinf(a[i])
                  		 - 1.0f);

			float bValue = sqrtf(cosf(b[i]) * cosf(b[i])
              		         + b[i] * b[i]
                 	         + sinf(b[i]) * sinf(b[i])
               		         - 1.0f);
				//testing out stupid people math equation
			c[i] = aValue + bValue; 
		}
	}
}
...
bool check(float *c, int n, float tolerance)
{
	...
	percentError = fabs((myAnswer - trueAnswer)/trueAnswer)*100.0; //change from "abs" to "fabs" because we are working with floating-point numbers
	
	if(percentError < tolerance) //lowercase "tolerance" so that the value of "Tolerance" gets passed into the function as "tolerance". The capital version is the global variable.
	{
		return(true);
	}
	else 
